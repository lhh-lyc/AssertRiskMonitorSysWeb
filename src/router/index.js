...
router.beforeEach((to, from, next) => {
  ...
  // }).catch(() => {})
  var userId = sessionStorage.getItem(commonkey.adminUserIdKey);
  // 获取菜单列表, 添加并全局变量保存
  http.get('/sys/menu/nav?userId=' + userId).then(({ data: res }) => {
    if (res.code != 200) {
      Vue.prototype.$message.error(res.msg)
      return next({ name: 'login' })
    }
    // 处理菜单，使数据符合显示
    let menuList = changeM(res.data.menuList)
  ...
  if (list && list.length > 0) {
    list.forEach(function(item){
      ...
      if(item.children && item.children.length > 0 ){
        item.children = changeM(item.children)
      }
    })
  }
  return list
